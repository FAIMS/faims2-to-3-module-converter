...
def tsprint(object, stream=None, indent=1, width=80, depth=None, *,compact=False, sort_dicts=True):
  import sys
  if stream is None:
    stream = sys.stdout
  
  stream.write(tsrepr(object, {}, 0))

def tsrepr(object, context, level):
  typ = type(object)
  if typ in {bytes, bytearray, int, float, complex}:
    return repr(object)
  ...
  if (issubclass(typ, dict) and r is dict.__repr__) or\
     (issubclass(typ, OrderedDict) and r is OrderedDict.__repr__):
    if not object:
      return "{}"
    ...
    items = object.items()
    for k, v in items:
      krepr = tsrepr(k, context, level)
      vrepr = tsrepr(v, context, level)
      append("%s: %s" % (krepr, vrepr))
    
    del context[objid]
    return "{\n%s%s,\n%s}" % (level*'  ', (",\n"+(level+1)*'  ').join(components), (level*'  '))
